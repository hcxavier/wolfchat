  ...
  return {
    h1: ({ children }) => (
      <div className="relative my-4 md:my-6 group w-full max-w-full overflow-hidden rounded-xl p-1">
        <h1 className="text-xl md:text-3xl font-black text-primary tracking-tighter relative z-10 pb-1 md:pb-2 drop-shadow-xl break-words">
          {children}
        </h1>
    ...
    h2: ({ children }) => (
      <h2 className="flex items-center gap-2 text-lg md:text-2xl font-bold text-primary/95 mt-4 md:mt-6 mb-2 pl-1 w-full max-w-full break-words">
    ...
    h3: ({ children }) => (
      <h3 className="text-base md:text-xl font-bold text-primary mt-4 md:mt-6 mb-2 flex items-center gap-2">
        <span className="flex items-center justify-center text-indigo-500 bg-indigo-500/10 w-6 h-6 md:w-8 md:h-8 rounded-lg shadow-[0_0_10px_rgba(99,102,241,0.2)]">
          <Hash size={14} className="md:w-4 md:h-4" />
        </span>
        <span className="bg-clip-text text-transparent bg-gradient-to-r from-primary to-indigo-500/80">
          {children}
        </span>
      </h3>
    ),
    h4: ({ children }) => (
      <h4 className="text-sm md:text-lg font-bold text-indigo-600 dark:text-indigo-200 mt-3 md:mt-5 mb-1 flex items-center gap-2">
        <span className="w-1.5 h-1.5 rounded-full bg-indigo-400" />
        {children}
      </h4>
    ),
    h5: ({ children }) => (
      <h5 className="text-sm md:text-base font-semibold text-primary/90 mt-2 md:mt-4 uppercase tracking-wide">
        {children}
      </h5>
    ),
    h6: ({ children }) => (
      <h6 className="text-xs md:text-sm font-semibold text-primary/70 mt-2 md:mt-4 italic">
        {children}
      </h6>
    ),
    p: ({ children }) => (
      <div className="whitespace-pre-wrap leading-relaxed text-primary/80 text-base mb-2 md:mb-3">
    ...
    li: ({ children }) => {
        return (
            <div className="group relative flex items-start gap-2 md:gap-3 py-1 px-1 rounded-lg hover:bg-primary/5 transition-colors duration-200">
                 ...
                 <div className="flex-1 text-primary/90 group-hover:text-primary leading-relaxed text-sm md:text-[0.95rem]">
                   {React.Children.map(children, child => {
                        if (typeof child === 'string') return processInlineTags(child);
    ...
    blockquote: ({ children }) => (
      <div className="relative my-3 md:my-6 group w-full">
          ...
          <div className="pl-4 md:pl-6 py-1">
              ...
              <div className="text-primary/90 font-serif italic leading-relaxed text-base">
                  {children}
              </div>
          </div>
      </div>
    ...
    a: ({ href, children }) => (
        <a 
            ...
            className="inline-flex items-center gap-1 text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 underline underline-offset-4 decoration-indigo-400/30 hover:decoration-indigo-300 transition-all font-medium"
        >
            {children}
    ...
    code: ({ node, className, children, ...props }: any) => {
        ...
        if (isInline) {
            return (
                <code className="px-1.5 py-0.5 rounded-md text-[0.85em] font-mono bg-primary/10 text-yellow-600 dark:text-yellow-300 border border-yellow-500/20 align-baseline shadow-[0_0_8px_rgba(234,179,8,0.1)] mx-0.5" {...props}>
                    {children}
                </code>
            );
        }
    ...
    img: ({ src, alt }) => (
        <div className="relative my-4 group overflow-hidden rounded-xl border border-primary/10 bg-surface-card">
            <img src={src} alt={alt} className="w-full h-auto object-cover opacity-90 group-hover:opacity-100 transition-opacity" />
            {alt && <div className="absolute bottom-0 left-0 right-0 bg-surface-main/60 backdrop-blur-sm p-2 text-xs text-primary/70 truncate">{alt}</div>}
            <div className="absolute top-2 right-2 p-1.5 bg-surface-main/40 backdrop-blur-md rounded-lg text-primary/50">
                <ImageIcon size={14} />
            </div>
        </div>
    ),
    ...
    table: ({ children }) => (
        <div className="w-full overflow-x-auto my-4 rounded-lg border border-primary/10">
    ...
    tr: ({ children }) => <tr className="border-b border-primary/5 last:border-0 hover:bg-primary/5 transition-colors">{children}</tr>,
    th: ({ children }) => <th className="p-3 text-left text-xs md:text-sm font-semibold text-indigo-600 dark:text-indigo-300 uppercase tracking-wider">{children}</th>,
    td: ({ children }) => <td className="p-3 text-sm text-primary/80 border-l border-primary/5 first:border-0">{children}</td>,
  };
};
      ...
      {parts.map((part, index) => {
        ...
        switch (part.type) {
            ...
            return (
              <MotionReveal key={index} delayIndex={counter.count++} shouldAnimate={shouldAnimate} onReveal={onReveal}>
                <div className="relative my-2 md:my-5 group w-full max-w-full rounded-xl p-1">
                  <h1 className="text-xl md:text-5xl font-black text-primary tracking-tighter relative z-5 pb-1 md:pb-3 drop-shadow-xl break-words">
          ...
          case 'subtitle':
            return (
              <MotionReveal key={index} delayIndex={counter.count++} shouldAnimate={shouldAnimate} onReveal={onReveal}>
                <h2 className="flex items-center gap-1.5 md:gap-3 text-base md:text-2xl font-bold text-primary/95 mt-2 md:mt-4 pl-1 w-full max-w-full break-words">
                  <div className="w-1 md:w-1.5 h-4 md:h-6 bg-gradient-to-b from-indigo-400 to-indigo-600 rounded-full shadow-[0_0_12px_rgba(99,102,241,0.8)] shrink-0" />
                ...
                <div className="my-2 md:my-5 relative overflow-hidden rounded-xl border border-red-500/30 bg-gradient-to-br from-red-500/10 to-red-500/5 p-2 md:p-5 group hover:border-red-500/50 transition-colors shadow-[inset_0_0_20px_rgba(239,68,68,0.05)] w-full max-w-full">
                   ...
                   <div className="relative flex gap-2 md:gap-4 items-start max-w-full">
                      ...
                      <div className="space-y-0.5 md:space-y-1.5 pt-0.5 min-w-0 flex-1">
                         ...
                         <div className="text-xs md:text-base text-red-800 dark:text-red-100/95 leading-relaxed font-medium break-words">
                            {processInlineTags(part.content)}
                         </div>
                      </div>
                   </div>
             ...
             return (
               <MotionReveal key={index} delayIndex={counter.count++} shouldAnimate={shouldAnimate} onReveal={onReveal}>
                 <div className="my-1.5 md:my-3 p-2 md:p-3 rounded-lg bg-teal-500/10 border border-teal-500/20 flex items-start gap-2 md:gap-3 backdrop-blur-sm shadow-sm hover:border-teal-500/40 transition-colors w-full max-w-full">
                    ...
                    <div className="text-[11px] md:text-sm text-teal-900 dark:text-teal-100/90 leading-relaxed break-words min-w-0 flex-1">
                       {processInlineTags(part.content)}
                    </div>
                 </div>
               </MotionReveal>
             ...
             return (
               <MotionReveal key={index} delayIndex={counter.count++} shouldAnimate={shouldAnimate} onReveal={onReveal}>
                 <div className="relative my-3 md:my-8 group w-full max-w-full">
                    <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-indigo-500 to-purple-500 rounded-full" />
                    <div className="absolute -left-3 -top-2 md:-top-3 p-1 md:p-1.5 bg-surface-card rounded-full border border-indigo-500/30 shadow-lg">
                       <Quote size={10} className="text-indigo-400 fill-indigo-400/20 md:w-[14px] md:h-[14px]" />
                    </div>
                    <div className="pl-4 md:pl-6 py-0.5 md:py-1">
                       <p className="text-xs md:text-xl text-primary/90 font-serif italic leading-relaxed drop-shadow-sm break-words">
                          "{processInlineTags(part.content)}"
                       </p>
                    </div>
                 </div>
             ...
             return (
                <MotionReveal key={index} delayIndex={counter.count++} shouldAnimate={shouldAnimate} onReveal={onReveal}>
                   <div className="relative my-2 md:my-4 w-full group max-w-full min-w-0">
                      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/[0.03] to-primary/[0.01] border border-primary/10 p-2 md:p-10 backdrop-blur-xl shadow-2xl transition-all duration-500 hover:border-brand-500/30 hover:shadow-brand-500/10 w-full">
                         ...
                         <div className="relative z-10 py-6 md:py-2 flex flex-col items-center justify-center text-center gap-2 md:gap-6 w-full max-w-full whitespace-normal">
                            <div className="p-1 md:p-3 rounded-full bg-brand-500/10 border border-brand-500/20 text-brand-600 dark:text-brand-300 group-hover:scale-110 group-hover:bg-brand-500/20 transition-all duration-300 shrink-0">
                            ...
                            <div className="space-y-1 md:space-y-4 w-full max-w-full px-1">
                                <h2 className="text-3xl md:text-5xl font-bold tracking-tight text-primary drop-shadow-lg break-words w-full leading-snug">
                                  {processInlineTags(part.content)}
                                </h2>
                                <div className="h-px w-10 md:w-32 mx-auto bg-gradient-to-r from-transparent via-primary/20 to-transparent" />
                            </div>
                         </div>
                      </div>
                   </div>
